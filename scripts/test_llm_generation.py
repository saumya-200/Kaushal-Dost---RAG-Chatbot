...
async def verify_language_mirroring_and_constraints():
    router = Router()
    ...
    for q in queries:
        ...
        stage, answer, meta = await router.route(q)
...
async def verify_conversation_history():
    router = Router()
    ...
    q1 = "What is UPSDM?"
    print(f"\nTurn 1 Query: '{q1}'")
    stage1, answer1, meta1 = await router.route(q1, history)
    ...
    stage2, answer2, meta2 = await router.route(q2, history)
...
async def verify_fallback_model():
    print("\n" + "="*80)
...
async def main():
    await verify_language_mirroring_and_constraints()
    await verify_conversation_history()
    await verify_fallback_model()
